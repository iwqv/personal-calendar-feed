...
- 标题短句优先：假期用 `🌴 节日放假 第N天/共N天`，补班用 `💼`，抢票用 `🚄`，礼物准备用 `🎁`，黄历用 `💡 今日宜忌`。普通节日直接写名称，节气统一用 `🌿`。
- 备注分清用途：假期写放假范围、补班和依据；提醒写具体要做的事；黄历写农历、宜忌、术语释义与来源。每段只用一个有明确含义的 emoji。
- 提醒语气亲切、信息准确。抢票提醒放在预计开售的前一晚，注明预售期只是推算；传统宜忌写明仅供民俗参考。日期变化时保留现有订阅地址，尽量保持同一日程的 `UID`。
- 礼物提醒由 `config.json` 的 `gift_rules` 明确列出，不根据节日名称自动推断；目前情人节、母亲节、父亲节、七夕节均提前 7 天在 09:00 提醒。用户可自行增删，不会给所有节日都加“送礼”要求。
...
- 12306 预售期和各站起售时间可能调整。当前配置按“含乘车当日 15 天”推算，假期首日前 15 天的晚上 20:00 提醒留意次日开售；购票前请在 12306 核对
